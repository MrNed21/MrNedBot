...
class RNG(commands.Cog):
    ...
    #magic 8ball command and aliase function 
    #for multiple ways to invoke commands
    #and list for random responses
    ...
    @commands.command(aliases=['treasurehunt'])
    async def search(self, ctx):
        """look for meaningless loot!"""
        responses = ["nothing",
                     "out that they wasted their time",
                     "new determination",
                     "some sand",
                     "out that joining discord.gg/4UHkHDq would be a good idea",
                     "the shovel they were using",
                     "!the treasure!",
                     "a coin",
                     ]
        await ctx.send(f'{ctx.message.author.mention} found {random.choice(responses)}.')
    ...
    @commands.command()
    async def cool(self, ctx, *, question):
        "Says if a user is cool."
        responses = ["cool",
                     "not cool.",
                     ]
        await ctx.send(f'{question} is {random.choice(responses)}')

    @commands.command()
    async def yelp(self, ctx, review):
        "yelp review"
        await ctx.send(f'{review}: {random.randint(1,10)}/10')
    ...
    async def catfact(self, ctx):
        "states one of 100 or so cat facts"
        responses = [
    ...
    "Researchers believe the word “tabby” comes from Attabiyah, a neighborhood in Baghdad, Iraq. Tabbies got their name because their striped coats resembled the famous wavy patterns in the silk produced in this.",
    ...
    "The largest cat breed is the Ragdoll. Male Ragdolls weigh between 12 and 20 lbs (5.4-9.0 k). Females weigh between 10 and 15 lbs.",
